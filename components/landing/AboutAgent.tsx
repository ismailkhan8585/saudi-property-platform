...
export default async function AboutAgent() {
  const settings = (await getHomePageData()).settings;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 items-center">
          ...
          <div className="flex flex-col items-center lg:items-start gap-6">
            <div className="relative">
              <div className="w-48 h-48 sm:w-56 sm:h-56 md:w-72 md:h-72 rounded-2xl overflow-hidden border-4 border-gold-400 shadow-2xl">
                <Image
                  ...
                  sizes="(max-width: 640px) 12rem, (max-width: 1024px) 18rem, 18rem"
                />
              </div>
              <div className="absolute -bottom-3 right-0 sm:-right-3 bg-gold-500 text-white rounded-xl px-3 sm:px-4 py-2 shadow-lg">
                <span className="font-price font-700 text-xl sm:text-2xl">{STATS.yearsExperience}+</span>
                <p className="text-xs">Years Exp.</p>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-2 sm:gap-4 w-full max-w-xs">
              {[
                { Icon: Home,  value: `${STATS.dealsClosed}+`, label: 'Deals Closed' },
                { Icon: Star,  value: '5.0',                   label: 'Rating' },
                { Icon: Award, value: `${STATS.propertiesListed}+`, label: 'Properties' },
              ].map(({ Icon, value, label }) => (
                <div key={label} className="min-w-0 bg-white rounded-xl p-2 sm:p-3 text-center border border-surface-border shadow-sm">
          ...
          <div>
            ...
            <h2 className="font-heading font-800 text-navy-700 text-2xl sm:text-3xl md:text-4xl mb-2">
            ...
            <div className="grid grid-cols-1 sm:flex sm:flex-wrap gap-3 mt-6">
              <a
                ...
                className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-3 rounded-xl font-600 transition-colors"
              >
              ...
              <a
                href={CALL_URL}
                className="flex items-center justify-center gap-2 bg-navy-500 hover:bg-navy-600 text-white px-5 py-3 rounded-xl font-600 transition-colors"
              >
              ...
              <Link
                href="/about"
                className="flex items-center justify-center gap-2 border border-navy-300 text-navy-600 hover:bg-navy-50 px-5 py-3 rounded-xl font-600 transition-colors"
              >
                View Full Profile
              </Link>
            </div>
