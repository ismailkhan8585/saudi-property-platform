...
export default async function HousesPage({ searchParams }: { searchParams: Promise<PageSearchParams> }) {
      ...
      <main className="min-h-screen bg-surface-secondary">
        <div className="bg-navy-700 pt-20 sm:pt-24 pb-8 sm:pb-10">
          <div className="max-w-7xl mx-auto px-4">
            <h1 className="font-heading font-800 text-white text-2xl sm:text-3xl md:text-4xl">Houses in Lahore</h1>
            <p className="text-white/60 text-sm sm:text-base mt-2">Single storey, double storey, and luxury houses across Lahore</p>
          </div>
        </div>
        <div className="max-w-7xl mx-auto px-4 py-5 sm:py-8">
          <div className="flex flex-col lg:flex-row gap-5 lg:gap-8">
            ...
            <Suspense fallback={<div className="flex-1 animate-pulse bg-gray-200 rounded-2xl h-96" />}>
              <PropertyResults searchParams={resolvedSearchParams} initialCategory="HOUSE" />
            </Suspense>
          </div>
        </div>
      </main>
